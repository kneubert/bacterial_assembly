...
then
 flexbar --reads $FQ/${base1}.fastq --reads2 $FQ/${base2}.fastq --threads $THREADS --adapter-min-overlap 4 --adapters $DIR/configs/adapter.fasta --min-read-length 35 --adapter-trim-end ANY -q BWA -qf i1.8 -qt 25 -1 --target flexbar/${ES}.ec >flexbar/${ES}.ec.fastq
fi
